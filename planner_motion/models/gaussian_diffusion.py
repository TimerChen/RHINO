...
from models.losses import InterLoss, GeometricLoss, RotationLoss
...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            ...
            from utils.utils import mytqdm as tqdm
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, control=False):
        ...
        if control:
            # TODO(jingxiao): check the control function
            assert False
            B, T, D = x_t.shape
...
class MotionDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, motion_rep, skip_text, pred_human, vel_loss, **kwargs):
    ...
    def training_losses(self, model, mask, t_bar, cond_mask, *args, **kwargs):
        ...
        # print("cond shape", kwargs["model_kwargs"]["cond"].shape ,target.shape)
        ...
        kwargs["model_kwargs"]["cond"] = kwargs["model_kwargs"]["cond"][:, :, :model.input_feats+model.input_h1_feats]
        # print("cond shape2", kwargs["model_kwargs"]["cond"].shape ,target.shape)

        # kwargs["x_start"] = target.reshape(B, T, -1)  # 82=60+22
        kwargs["x_start"] = target.reshape(B, T, -1)[:, :, :model.input_feats+model.input_h1_feats]  # 82=60+22
        items = super().training_losses(self._wrap_model(model), *args, **kwargs)
        prediction = items["pred"].reshape(B, T, -1)
        target = items["target"].reshape(B, T, -1)

        timestep_mask = (kwargs["t"] <= t_bar).float()  # * cond_mask[0, 0]

        # ignore loss of history
        if self.skip_text:
            mask = mask - cond_mask.view(B,T,1,1)

        losses = {}
        if self.pred_human:
            loss_human_manager = RotationLoss("Human", "l2", 8, normalizer=self.normalizer, weights=None, normal_slice=slice(0, model.input_feats))
            loss_human_manager.forward(prediction[..., :model.input_feats], target[..., :model.input_feats], mask[...,0,:], timestep_mask)
            losses.update(loss_human_manager.losses)
            
        loss_h1_manager = RotationLoss(
            "H1", "l2", 8, 
            normalizer=self.normalizer, 
            weights={"VEL": self.vel_loss}, 
            normal_slice=slice(model.input_feats, model.input_feats + model.input_h1_feats)
        )
        loss_h1_manager.forward(
            prediction[...,model.input_feats:model.input_feats + model.input_h1_feats], 
            target[...,model.input_feats:model.input_feats + model.input_h1_feats], 
            mask[...,0,:], 
            timestep_mask,
        )
        losses.update(loss_h1_manager.losses)
        
        if self.pred_human:
            losses["total"] = loss_human_manager.losses["Human"] + loss_h1_manager.losses["H1"]
        else:
            losses["total"] = loss_h1_manager.losses["H1"]
