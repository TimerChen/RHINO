...
def cont6d_to_axis_angle(cont6d):
    """
    Args:
        matrix: contd vectors as tensor of shape (..., 6).
    Returns:
        Rotations given as a vector in axis angle form, as a tensor
            ...
            direction.
    """
    return matrix_to_axis_angle(cont6d_to_matrix(cont6d))
...
def qbetween(v0, v1):
    ...
    # =============== TODO: Jingxiao: hack to avoid NaNs
    q_norm = torch.norm(q, dim=-1)
    if (q_norm < 1e-5).any():
        print("v0", v0.shape, q_norm.shape)
        if v0.shape[0] < q_norm.shape[0]:
            v1[q_norm < 1e-5] = v1[q_norm < 1e-5] + 1e-5
        else:
            v0[q_norm < 1e-5] = v0[q_norm < 1e-5] + 1e-5
        v = torch.cross(v0, v1)
        w = torch.sqrt((v0 ** 2).sum(dim=-1, keepdim=True) * (v1 ** 2).sum(dim=-1, keepdim=True)) + (v0 * v1).sum(dim=-1,
                                                                                                                keepdim=True)
        q = torch.cat([w, v], dim=-1)
        assert not (torch.norm(q, dim=-1, keepdim=True) < 1e-5).any(), "try to fix the NaNs, but failed"
    # =========================
