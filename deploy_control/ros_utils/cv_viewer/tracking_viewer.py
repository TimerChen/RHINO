...
def render_sk(left_display, img_scale, obj, color, BODY_BONES, lost_warning=False):
    # Draw skeleton bones
    ...
    # Skeleton joints
    for i, kp in enumerate(obj.keypoint_2d):
        cv_kp = cvt(kp, img_scale)
        if(0 < cv_kp[0] < left_display.shape[1] and 0 < cv_kp[1] < left_display.shape[0]):
            cv2.circle(left_display, (int(cv_kp[0]), int(cv_kp[1])), 7, color, -1)
            # cv2.putText(left_display, f"{i}", (int(cv_kp[0]), int(cv_kp[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 200), 2, cv2.LINE_AA)

        elif lost_warning and (BODY_BONES == sl.BODY_18_BONES and (1 <= i <= 7)):
            # draw warning if sholder is out of frame
            h, w = left_display.shape[:2]
            cv2.putText(left_display, "LOST", (w//2, h//2), cv2.FONT_HERSHEY_SIMPLEX, 6, (0, 0, 200), 20, cv2.LINE_AA)



def render_2D_body(left_display, img_scale, objects, is_tracking_on, body_format):
    '''
    Parameters
        ...
        if render_object(obj, is_tracking_on):
            if len(obj.keypoint_2d) > 0:
                ...
                if body_format == sl.BODY_FORMAT.BODY_18:
                    render_sk(left_display, img_scale, obj, color, sl.BODY_18_BONES, lost_warning=True)
...
CLS_NAME = {0: 'person', 1: 'bicycle', 2: 'car', 3: 'motorcycle', 4: 'airplane', 5: 'bus', 6: 'train', 
...
def render_2D_obj(left_display, img_scale, objects, is_tracking_on, scenario="dining"):
    CLS_NAME = CLS_MAP[scenario]
    overlay = left_display.copy()
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            text = f"class {obj.raw_label} {CLS_NAME[obj.raw_label]} {obj.confidence:.2f}"
            ...
            if np.isfinite(obj.position[0]):
                text = str(round(abs(obj.position[0]), 1)) + "M"
