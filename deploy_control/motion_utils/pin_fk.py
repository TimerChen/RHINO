...
class PinFk:
    ...
    def fk(
        ...
        for point_name, parent_point_name in interested_points:
            frame_id = self.robot.model.getFrameId(point_name)
            point_position = self.robot.data.oMf[frame_id].translation
            
            # get its parent link:
            parent_point_frame_id = self.robot.model.getFrameId(parent_point_name)
            parent_point_position = self.robot.data.oMf[parent_point_frame_id].translation

            point_positions[point_name] = point_position, parent_point_position
        if self.viz:
